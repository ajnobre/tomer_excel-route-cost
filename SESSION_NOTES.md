...
Quote sheet (visible):
  ...
  - 1 editable input: Discount (%) — default 30%, blue background like dropdowns
  - Tonnage warning row (row 13): orange banner when using unconfirmed default weight
  - Results table: 3 rows (Cochin, IN / Tuticorin, IN / Colombo, LK)
    Columns (9): Source, Product Code, FOB Price/Unit, **Disc. FOB/Unit**,
             Freight/Container, Units/Container, Freight/Unit, Total Cost/Unit, Transit Time
  - Total Cost = Discounted FOB + Freight/Unit (NOT full FOB)
  - Conditional formatting: amber tint on FOB, Disc. FOB, Units, Total when weight unconfirmed
...
6. **Client opens Excel**, selects 7 dropdowns + optionally adjusts discount % → weight tier auto-derives from destination → formulas do MATCH/INDEX lookups → results appear instantly
7. **Discount applied** — Disc. FOB = FOB × (1 - discount%). Total Cost uses discounted FOB, not full FOB.
8. **Warning system** alerts when tonnage is unconfirmed (orange banner + amber result cells)
9. **Client compares** all 3 origin rows (Cochin, Tuticorin, Colombo) to decide which gives the best deal
...
1. **B11 cell** — displays "23 MT (UNCONFIRMED)" + conditional formatting turns light orange background with orange text
2. **Row 13 warning banner** — orange background (#E67E22), white text: *"WARNING: No confirmed weight data for this destination. Using default 23 MT. Verify before quoting."*
3. **Result cells** — pale amber conditional formatting on FOB Price, Disc. FOB, Units/Container, and Total Cost columns
...
13. **Discount column** — editable B12 input (default 30%), Disc. FOB shown separately, Total Cost = Disc. FOB + Freight/Unit
14. **Orange warning tones** — warning banner and B11 use orange (#E67E22) instead of red for a softer visual

## Validation Results (Session 6)
...
| Formula arithmetic simulation (3 origins, discount at 30%, auto-derived weight) | PASS |
| Quote sheet formulas (K4-K13, 3 result rows, B11 auto-derive, B12 discount, A13 warning) | PASS |
...
Repository: https://github.com/ajnobre/tomer_excel-route-cost
  build_quotation_tool.py                  <- Python generator script (v5, discount + orange warnings)
...
- Conditional formatting uses FormulaRule with `$K$13=0` to detect unconfirmed tonnage (orange tones)
- EUR/USD exchange rate input removed (ALL IN values are all USD)
- Discount formula: `Disc. FOB = C{row} * (1 - $B$12)` where B12 defaults to 0.30 (30%)
