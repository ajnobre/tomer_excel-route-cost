...
FREIGHT_FILE = find_file('Freight*.xlsx', 'Freight')
...
def read_freight(filepath):
    """Read freight data, aggregate charges per route, pick cheapest per country+dest."""
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        route_charges[key]['transit'] = transit or ''
        if rate:
            if currency == 'USD':
    ...
    for (origin, dest), data in route_charges.items():
        ...
        approx_total = data['usd'] + data['eur'] * 1.08
        ck = (country, dest)
        if ck not in country_map or approx_total < country_map[ck]['approx']:
            transit_days = 0
            tm = re.search(r'(\d+)', str(data['transit']))
            if tm:
                transit_days = int(tm.group(1))
            country_map[ck] = {
                'country': country,
                'origin': origin,
                'destination': dest,
...
def _build_quote_sheet(ws):
    ...
    rate.fill = INPUT_FILL
    rate.font = INPUT_FONT
    ...
    ws['A20'] = (
        ...
        'IF(AND(K7>0,K8>0,K9>0,K10>0),"",'
        'IF(AND(K7=0,K8>0),"Product not available from India.",'
        'IF(AND(K7>0,K8=0),"Product not available from Sri Lanka.",'
        'IF(AND(K9=0,K10>0),"No freight route available from India to this destination.",'
        'IF(AND(K9>0,K10=0),"No freight route available from Sri Lanka to this destination.",'
        'IF(AND(K9=0,K10=0),"No freight routes available to this destination.",""))))))))'
    )
    ws['A20'].font = WARNING_FONT

    # ── Full descriptions (reference rows) ─────────────────────────────────
    ws.row_dimensions[21].height = 8
    ws['A22'].value = 'India - Full Description'
    ws['A22'].font = DETAIL_FONT_B
    ws.merge_cells('B22:H22')
    ws['B22'] = '=IF(NOT(K6),"",IF(K7=0,"-",INDEX(IN_Descs,K7)))'
    ws['B22'].font = DETAIL_FONT

    ws['A23'].value = 'Sri Lanka - Full Description'
    ws['A23'].font = DETAIL_FONT_B
    ws.merge_cells('B23:H23')
    ws['B23'] = '=IF(NOT(K6),"",IF(K8=0,"-",INDEX(SL_Descs,K8)))'
...
def _write_result_row(ws, row, label, prefix, is_alt=False):
    """Write one result row (India or Sri Lanka) with lookup formulas."""
    fill = ALT_ROW_FILL if is_alt else PatternFill()
    mr = 'K7' if prefix == 'IN' else 'K8'     # product match ref
    fr = 'K9' if prefix == 'IN' else 'K10'     # freight match ref
